...
```
## Introduction
In this vignette we will explore the function `get_bags()`. This function is used internally in both `aurora_pheno()` and `aurora_GWAS()` but can be also accessed upon installing *aurora*. The function takes a dataset of microbial strains and returns a bootstrapped dataset that is adjusted for the population structure. `get_bags()` has many possible uses and we will demonstrate a few here.
...
```
The last step is to calculate a median for all the feature importances and the number of times it was used in the 10 models. Additionally, the median of the 10 classification probabilities will also be calculated. This will allow us to identify potentially mislabeled strains.
```{r}
