...
```
## Introduction
Pangenome GWAS (panGWAS) is a genetic analysis method that investigates the relationship between genetic variations (genes in this case) across an entire species' genome and observable traits. PanGWAS relies on the construction of pangenomes and phylogenetic trees which are the main input into the analysis.
## Objective
...
The fact that a classifier performs well on a dataset does not mean that there are no mislabelled strains. You can examine the figures stored in "plot" variable to see the summary of labels predicted by each classifier.
```{r, eval = FALSE}
